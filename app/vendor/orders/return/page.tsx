...
export default function ReturnPage() {
    const [returnreq, setReturnReq] = useState<Return[]>([])
    ...
    // Actions component
    const ActionsCell = ({ reason }: { reason: Return }) => {
        return (
            <DropdownMenu>
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex flex-wrap gap-1 text-center">
                        {orders?(
                           <span className='text-center ml-4'> {orders.netTotal+" ₹"}</span>
                        ):(
                            "NULL"
                        )}
                    </div>
    ...
    // Load returnreq from API
    const loadComplaints = async () => {
        setLoading(true)
        try {
            const api = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api'
            const response = await axios.get(`${api}/vendor/return?page=${page}&limit=5`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
            setReturnReq(response.data.data || [])
            console.log(response.data)
            setPageCount(response.data.meta.totalPages || 1)
            console.log(response.data.meta.totalPages)
        } catch (error) {
            console.error('Error fetching returnreq:', error)
            Swal.fire({
                title: "Error",
                text: "Failed to load returnreq. Please try again later.",
                icon: "error",
                draggable: true
            })
        } finally {
    ...
    const deleteComplaint = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                const api = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api'
                const response = await axios.delete(`${api}/vendor/return/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                })
    ...
    // Edit reason
    const openEditDialog = (reason: Return) => {
    ...
    const updateComplaint = async () => {
        ...
        try {
            const api = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api'
            await axios.patch(`${api}/vendor/return/${selectedComplaint.id}`, {
              ...
              <>
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Manage and track all the returns
                  </p>
                </div>
              </>
            )}
                ...
                <DialogContent className="sm:max-w-[425px]">
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid gap-2">
                            <Label htmlFor="reason">Return</Label>
                            <textarea
                                id="reason"
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={updateComplaint}>Update Return</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
